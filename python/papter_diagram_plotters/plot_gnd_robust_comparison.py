#!/usr/bin/env python3
"""
Paper figures: GND vs Gaussian and common SLAM robust kernels.

Demonstrates that GND (high β) approximates a set-theoretic / uniform-on-a-set
view (flat core, bounded influence), unlike smooth M-estimators.

Run from repo root:
  python python/diagram_plotters/plot_gnd_robust_comparison.py
...
# Match GND σ for other kernels (g2o δ in residual units).
DELTA = SIGMA
...
def plot_cost_vs_residual() -> None:
    ...
    curves = [
        ...
        (f"GND kernel ($\\sigma$={SIGMA}, $\\beta$={BETA_LOW})", gnd_kernel(e2, SIGMA, BETA_LOW, LNC),
         dict(color="#1f77b4", lw=2.0)),
        (f"GND kernel ($\\sigma$={SIGMA}, $\\beta$={BETA_HIGH})", gnd_kernel(e2, SIGMA, BETA_HIGH, LNC),
         dict(color="#0b4f8a", lw=2.0, ls="--")),
        (f"GND edge ($\\beta$={BETA_LOW}, $\\Omega/4$)", gnd_edge(e2, BETA_LOW, LNC),
    ...
    axes[1].set_title(f"Core region ($|r| \\leq 2\\sigma$) — flat top of GND")
    axes[1].set_xlim(-2 * SIGMA, 2 * SIGMA)
    ymax = gnd_kernel(SIGMA * SIGMA, SIGMA, BETA_LOW, LNC) * 1.15
    axes[1].set_ylim(-0.02, ymax)

    fig.suptitle(
        "Robust costs: GND flat core vs smooth M-estimators (g2o formulations, $\\delta=\\sigma$)",
        y=1.02,
    )
    fig.tight_layout()
...
def plot_influence_weights() -> None:
    ...
    curves = [
        ...
        (f"GND $\\beta$={BETA_LOW}", gnd_kernel_deriv(e2, SIGMA, BETA_LOW), dict(color="#1f77b4", lw=2)),
        (f"GND $\\beta$={BETA_HIGH}", gnd_kernel_deriv(e2, SIGMA, BETA_HIGH), dict(color="#0b4f8a", lw=2, ls="--")),
    ...
    ax.set_title("Influence functions: GND stays active in core, then drops; Tukey/Saturated hard-zero")
...
def plot_tail_log() -> None:
    """Log-scale tails: GND power growth vs logarithmic Cauchy."""
    ...
    series = [
        ...
        (f"GND $\\beta$={BETA_LOW}", gnd_kernel(e2, SIGMA, BETA_LOW, LNC)),
        (f"GND $\\beta$={BETA_HIGH}", gnd_kernel(e2, SIGMA, BETA_HIGH, LNC)),
    ...
    ax.set_title("Tail behaviour beyond $\\sigma$: power-like (GND) vs bounded/log (common robust kernels)")
...
def plot_2d_level_sets() -> None:
    """2D isotropic level sets: GND high-β approaches a box (set), Gaussian is round."""
    lim = 4.0 * SIGMA
    n = 200
    ...
    levels_gnd_low = [
        gnd_kernel(SIGMA * SIGMA * 0.25, SIGMA, BETA_LOW, LNC),
        gnd_kernel(SIGMA * SIGMA, SIGMA, BETA_LOW, LNC),
        gnd_kernel((2 * SIGMA) ** 2, SIGMA, BETA_LOW, LNC),
    ]
    levels_gnd_high = levels_gnd_low
    ...
    panels = [
        ...
        (f"GND $\\sigma$={SIGMA}, $\\beta$={BETA_LOW}", gnd_kernel,
         dict(sigma=SIGMA, beta=BETA_LOW, lnc=LNC), levels_gnd_low),
        (f"GND $\\sigma$={SIGMA}, $\\beta$={BETA_HIGH}", gnd_kernel,
         dict(sigma=SIGMA, beta=BETA_HIGH, lnc=LNC), levels_gnd_high),
    ]
    ...
    fig.suptitle("2D level sets (unit $\\Omega$): GND $\\beta\\uparrow$ squashes contours toward a set / box", y=1.02)
    fig.tight_layout()
...
def plot_set_limit() -> None:
    """GND β → ∞ limit vs ideal set indicator and uniform-on-set intuition."""
    ...
    fig, ax = plt.subplots(figsize=(8, 4.5))
    for b in betas:
        ax.plot(r, gnd_kernel(e2, SIGMA, b, LNC), label=f"GND $\\beta={b}$", lw=1.8 if b == BETA_HIGH else 1.2)
    ax.plot(
        r,
        set_indicator_cost(r, SIGMA, outside=np.max(gnd_kernel(SIGMA * SIGMA, SIGMA, BETA_HIGH, LNC))),
        "k:",
        lw=2,
    ...
    ax.set_title(f"GND family at $\\sigma={SIGMA}$: increasing $\\beta \\to$ flat uniform-like core (set view)")
    ax.legend(loc="upper center", ncol=2)
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    _save(fig, "gnd_set_limit.png")
    plt.close(fig)


def main() -> None:
    _style()
    print(f"GND paper plots → {OUTPUT_DIR}")
