...
def sequential_spaced_samples(
    candidates: Sequence[Tuple[int, ...]],
    threshold: float,
    n: int,
    existing_samples: Sequence[Tuple[int, ...]],
) -> List[Tuple[int, ...]]:
    """
    Chebyshev (L∞) spacing: drop any point that lies strictly **closer** than ``threshold``
    to a reference. First drop points too close to ``existing_samples``, sample one uniformly
    from what remains, then repeat ``n - 1`` times: drop points too close to the last pick,
    sample again. Returns ``n`` distinct picks (inputs are treated as 3D integer coordinates).
    """
    if n < 1:
        return []
    thr = float(threshold)

    def cheb(a: Tuple[int, ...], b: Tuple[int, ...]) -> float:
        a = tuple(map(int, a))
        b = tuple(map(int, b))
        return float(max(abs(a[i] - b[i]) for i in range(3)))

    def far_from(p: Tuple[int, ...], refs: Sequence[Tuple[int, ...]]) -> bool:
        return all(cheb(p, s) >= thr for s in refs)

    ex = [tuple(map(int, p)) for p in existing_samples]
    pool = [tuple(map(int, c)) for c in candidates if far_from(tuple(map(int, c)), ex)]
    random.shuffle(pool)
    out: List[Tuple[int, ...]] = []
    for _ in range(n):
        if not pool:
            raise ValueError(
                f"sequential_spaced_samples: need {n} picks but pool exhausted "
                f"(threshold={threshold}, got {len(out)})."
            )
        pick = pool.pop(random.randrange(len(pool)))
        pool = [c for c in pool if cheb(c, pick) >= thr]
        out.append(pick)
    return out
...
def trajectory_generation(
    ...
    **Inflation (when enabled)** uses **only the current leg**: within each midpoint round,
    ...
    Placement uses Chebyshev separation ``thr = inflate + 1`` between path centers (same
    scale as ``inflate_path_into_voxels``). ``sequential_spaced_samples`` helps draw starts
    and (when needed) same-face goals; occupancy always includes every other agent's
    current vertex, not only earlier shuffle order this leg.
    If a bounce/opposite **goal** lies too close to existing endpoints, goals are taken
    from a same-face pool via that sequential sampler instead of a plain shuffle.
    ...
    # Chebyshev separation of path *centers* must exceed inflate (inflated tubes touch at inflate+1).
    thr = int(inflate) + 1
    ...
    for round_idx in range(n_rounds):
        ...
        def occupied_vertices(bid_key) -> List[Tuple[int, ...]]:
            """
            All lattice points that are already 'taken' for spacing purposes while
            planning ``bid_key``: committed (p,q) pairs this leg, plus every *other*
            agent's current vertex (start of this leg until they get a new goal).
            """
            occ: set[Tuple[int, ...]] = set(placed_round)
            for ob in bot_ids:
                if ob == bid_key:
                    continue
                if ob not in ends:
                    continue
                occ.add(tuple(map(int, ends[ob])))
            return list(occ)
        ...
        for bid in order:
            ...
            success = False
            while not success:
                occ = occupied_vertices(bid)
                q_spacing_ref = occ

                if round_idx == 0:
                    if not loc:
                        break
                    # Both start and opposite goal must clear prior placements; filter before sampling.
                    loc_ok = [
                        c
                        for c in loc
                        if not any(
                            linf(tuple(map(int, c)), r) < thr
                            or linf(tuple(map(int, opposite(c))), r) < thr
                            for r in placed_round
                        )
                    ]
                    try:
                        p, = sequential_spaced_samples(
                            loc_ok, float(thr), 1, existing_samples=placed_round
                        )
                    except ValueError:
                        break
                else:
                    p = tuple(map(int, ends[bid]))
                    if any(linf(p, r) < thr for r in occ):
                        raise ValueError(
                ...
                if round_idx == 0:
                    ...
                    face_extra = [c for c in goals_sharing_face(q_seed) if c != qt]
                    face_cands = [qt] + face_extra
                    seed_too_close = any(linf(qt, r) < thr for r in q_spacing_ref)
                    if seed_too_close:
                        try:
                            n_try = min(len(face_cands), 128)
                            qopts = sequential_spaced_samples(
                                face_cands,
                                float(thr),
                                n_try,
                                existing_samples=q_spacing_ref,
                            )
                        except ValueError:
                            random.shuffle(face_cands)
                            qopts = list(face_cands)
                    else:
                        random.shuffle(face_cands)
                        qopts = list(face_cands)
                for q in qopts:
                    ...
                    if round_idx == 0:
                        if any(
                            linf(p, r) < thr or linf(q, r) < thr for r in occ
                        ):
                            continue
                    elif any(linf(q, r) < thr for r in q_spacing_ref):
                        continue
                    ...
                    success = True
                    break
                if round_idx == 0 and not success:
                    loc = [c for c in loc if c != p]
