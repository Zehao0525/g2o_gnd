...
void SlamSystem::landmarkEstimates(std::vector<Vector2d>& m,
...
void SlamSystem::getSceneEstimates(Eigen::Vector3d& x,
...
void SlamSystem::platformEstimate(Eigen::Vector3d& x) const {
  EstimateType pose;
  Base::platformEstimate(pose);
  x = pose.toVector();
}

void SlamSystem::getSceneEstimatesWithP(Eigen::Vector3d& x, Eigen::Matrix2d& P,
                                           std::vector<Eigen::Vector2d>& /*m*/,
...
void SlamSystem::handleRangeBearingObservationEvent(
...
void SlamSystem::handleGPSObservationEvent(GPSObservationEvent event) {
  auto* gpsObservation = new EdgePlatformLocPriorGGD;
  gpsObservation->setVertex(0, currentPlatformVertex_);
  gpsObservation->setMeasurement(event.value);
  gpsObservation->ggdSetInformation(event.covariance.inverse(), 8);
