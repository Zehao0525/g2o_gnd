// GGD beta parameter sweep on the tutorial_w_references correlated-GPS problem.
...
struct BetaSweepConfig {
  ...
  std::string outputBaseDir = "test_results/ggd_beta_study/correlated_gps";
};
...
BetaSweepConfig loadConfig(const std::string& configPath) {
  ...
  if (j.contains("optimization")) {
    ...
    cfg.optimization.maxIterations =
        o.value("max_iterations", o.value("ggd_active_iterations", cfg.optimization.maxIterations));
...
void assignGgdKernels(SparseOptimizer& optimizer, double beta, const KernelParams& params,
                      bool* ggdActivePtr) {
  for (auto& edgePair : optimizer.edges()) {
    ...
    gps->setRobustKernel(new ToggelableGGDKernel(params.ggdBound, beta, params.ggdLnc,
...
RunMetrics runGgdBeta(SparseOptimizer& optimizer, const Simulator& simulator, double beta,
                      const KernelParams& kernelParams, const OptimizationConfig& optCfg,
                      bool& ggdActive, std::ostream& traceOut, int testIdx) {
  RunMetrics metrics;
  metrics.variant = "ggd";
  metrics.beta = beta;

  ggdActive = false;
  assignGgdKernels(optimizer, beta, kernelParams, &ggdActive);
  metrics.chi2Initial = currentChi2(optimizer);

  metrics.warmup =
      runOptimizationPhase(optimizer, optCfg.ggdWarmupIterations, optCfg.chi2RelTol,
                             /*collectStats=*/true, &traceOut, testIdx, "ggd", beta, "warmup");

  ggdActive = true;
  metrics.active = runOptimizationPhase(optimizer, optCfg.maxIterations, optCfg.chi2RelTol,
                                        /*collectStats=*/true, &traceOut, testIdx, "ggd", beta,
                                        "active");
...
int main(int argc, char* argv[]) {
  ...
  const std::string defaultConfigPath =
      "experiments/pilots/ggd_beta_study/config/beta_sweep_config.json";
  ...
  for (int testIdx = 0; testIdx < simCfg.numTests; ++testIdx) {
    ...
    for (double beta : cfg.betaValues) {
      resetEstimates(optimizer, simulator);
      RunMetrics ggdRun =
          runGgdBeta(optimizer, simulator, beta, kernelParams, optCfg, ggdActive, traceOut, testIdx);
      writeCsvRow(summary, testIdx, ggdRun);
      writeCsvRow(aggregate, testIdx, ggdRun);
      optimizer.save((testDir + "/" + outputTag(ggdRun) + ".g2o").c_str());
      cerr << "test " << testIdx << " beta=" << beta << endl;
      cerr << "  chi2=" << ggdRun.chi2Final << " mean_pose_err=" << ggdRun.meanPoseErr
           << " time_s=" << ggdRun.totalTimeS() << " lm_inners=" << ggdRun.totalLmInners() << endl;
    }
