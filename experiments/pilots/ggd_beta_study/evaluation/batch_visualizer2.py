...
Default results directory:
  test_results/ggd_beta_study/references_robust_kernels

Files per test_k/:
  twb_gt.g2o, twb_before.g2o, twb_{gauss,gnd|ggd,huber,...}.g2o

Run full statistical evaluation (APE + MSE, kernel ranking, seed analysis):
  python3 experiments/pilots/ggd_beta_study/evaluation/batch_visualizer2.py

Trajectory overlay for one seed (optional):
  python3 experiments/pilots/ggd_beta_study/evaluation/batch_visualizer2.py --plot-test 0 --plot-kernel ggd
"""
...
DEFAULT_RESULTS_ROOT = Path("test_results/ggd_beta_study/simulated_gps_like_test/30_p2_sig25_w_robust_kernels")
# Map CLI/kernel names to on-disk twb_*.g2o stems (legacy results use "gnd").
KERNEL_FILE_ALIASES = {
    "gaussian": ("gauss",),
    "ggd": ("ggd", "gnd"),
    "gnd": ("gnd", "ggd"),
...
def _resolve_twb_path(test_dir: Path, kernel: str) -> Path:
    stems = KERNEL_FILE_ALIASES.get(kernel, (kernel,))
    if isinstance(stems, str):
        stems = (stems,)
    for stem in stems:
        path = test_dir / f"twb_{stem}.g2o"
        if path.is_file():
            return path
    raise FileNotFoundError(
        f"No twb_*.g2o for kernel '{kernel}' in {test_dir} (tried stems {stems})"
    )
...
def plot_kernel_vs_baseline(
    ...
    base_path = _resolve_twb_path(test_dir, baseline)
    kernel_path = _resolve_twb_path(test_dir, kernel)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--plot-kernel", default="ggd", help="Kernel to compare against baseline in plot")
