#!/usr/bin/env python3
"""
Evaluate GGD beta oscillation / convergence study outputs.
...
Run from repo root:
  python experiments/pilots/ggd_beta_study/evaluation/evaluate_beta_sweep.py
  python experiments/pilots/ggd_beta_study/evaluation/evaluate_beta_sweep.py --results test_results/ggd_beta_study/correlated_gps
"""
...
DEFAULT_RESULTS_ROOT = Path("test_results/ggd_beta_study/correlated_gps_tight")
...
def condition_label(variant: str, beta: Optional[float]) -> str:
    ...
    if abs(beta - round(beta)) < 1e-9:
        return f"GGD β={int(round(beta))}"
    return f"GGD β={beta:g}"
...
def estimate_g2o_path(results_root: Path, test_idx: int, row: Dict[str, Any]) -> Path:
    ...
    beta = row["beta"]
    if beta is None or np.isnan(beta):
        raise ValueError(f"Missing beta for ggd row in test {test_idx}")
...
def _ggd_only(summary: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def plot_metric_vs_beta(
    ...
    rows = _ggd_only(summary) if ggd_only else summary
    if not rows:
...
def plot_convergence_panel(summary: Sequence[Dict[str, Any]]) -> None:
    ggd = _ggd_only(summary)
    if not ggd:
        print("Skipping convergence_panel.png: no GGD rows")
        return
    ...
    for ax, (field, ylabel, title) in zip(axes.ravel(), metrics):
        xs = [float(r["beta"]) for r in ggd]
        med = [_extract(summary, r["condition"], f"{field}_median") for r in ggd]
        q25 = [_extract(summary, r["condition"], f"{field}_q25") for r in ggd]
        q75 = [_extract(summary, r["condition"], f"{field}_q75") for r in ggd]
...
def plot_oscillation_panel(summary: Sequence[Dict[str, Any]]) -> None:
    ...
    for ax, (metric_base, ylabel) in zip(axes.ravel(), metrics):
        xs = [float(r["beta"]) for r in ggd]
        med = [_extract(summary, r["condition"], f"{metric_base}_median") for r in ggd]
        q25 = [_extract(summary, r["condition"], f"{metric_base}_q25") for r in ggd]
        q75 = [_extract(summary, r["condition"], f"{metric_base}_q75") for r in ggd]
        med_arr = np.asarray(med)
...
def print_oscillation_summary_table(
    ...
    print("\n=== GGD β oscillation study (median [IQR] over seeds) ===")
    header = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate GGD beta oscillation study outputs.")
    parser.add_argument(
        "--results",
        type=Path,
