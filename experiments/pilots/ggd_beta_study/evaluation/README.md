...
Offline metrics and paper figures for **GGD / GGD** robust-kernel experiments on Tutorial_slam2d (GPS-like / absolute priors), including beta sweeps and GPS-period sweeps.

Default C++ result trees live under `test_results/ggd_beta_study/`. Script outputs go to `output/` next to these scripts.
...
| `summarize_gps_period_gaussian_vs_ggd.py` | Cross-period Gaussian vs GGD APE summary (wins + t / Wilcoxon) |
...
```text
test_results/ggd_beta_study/
  references_robust_kernels/          # or references_robust_kernels_period_{P}/
    test_*/twb_{gauss,ggd,...}.g2o, twb_gt.g2o, kernel_summary.csv
...
python3 experiments/pilots/ggd_beta_study/evaluation/evaluate_beta_sweep.py
```
