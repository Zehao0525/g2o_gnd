...
def evaluate_batch_results(root_dir="gnd_test_results", num_tests=30, save_path=None):
    ...
    for i in range(num_tests):
        ...
        try:
            ...
            poses_ggd = read_se2_vertices(file_ggd)
...
def plot_ggd_vs_gps(root_dir, i, gps_filename=None, save_path=None, show=True):
    """
    Plot GGD and GPS/GT trajectories for test i (root_dir/test_i).
    ...
    if not os.path.exists(file_ggd):
        raise FileNotFoundError(f"GGD file not found: {file_ggd}")

    # ---- Load poses (expects your existing helper) ----
    poses_ggd = read_se2_vertices(file_ggd)
    poses_gps = read_se2_vertices(file_gps)

    if poses_ggd is None or poses_gps is None:
        raise ValueError("Missing or empty pose data for GGD and/or GPS/GT.")
    ...
    ggd = to_dict(poses_ggd)
    gps = to_dict(poses_gps)

    common_ids = sorted(set(ggd.keys()) & set(gps.keys()))
    if not common_ids:
        raise ValueError("No common vertex IDs between GGD and GPS/GT trajectories.")

    ggd_xy = np.array([[ggd[k][0], ggd[k][1]] for k in common_ids], dtype=float)
    gps_xy = np.array([[gps[k][0], gps[k][1]] for k in common_ids], dtype=float)

    # ---- Plot ----
    fig, ax = plt.subplots()
    ax.plot(gps_xy[:, 0], gps_xy[:, 1], label="GPS trajectory")
    ax.plot(ggd_xy[:, 0], ggd_xy[:, 1], label="GGD trajectory")
    ax.scatter([gps_xy[0, 0]], [gps_xy[0, 1]], label="Start", zorder=5)
    ax.set_aspect("equal", adjustable="datalim")
    ax.set_title(f"GGD vs GPS — test_{i}")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
...
if __name__ == "__main__":
    df_results = evaluate_batch_results(root_dir="test_results/tutorial_slam2d/exp1_test_results_2", save_path="test_results/tutorial_slam2d/exp1_test_results_2/ape_results.pkl")
    print(df_results)
    ...
    plot_ggd_vs_gps(root_dir="test_results/tutorial_slam2d/exp1_test_results_2", i = 22)