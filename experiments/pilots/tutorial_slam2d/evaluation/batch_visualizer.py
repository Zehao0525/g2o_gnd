...
def evaluate_batch_results(root_dir="gnd_test_results", num_tests=30, save_path=None, percentile = 1.0):
    ...
    for i in range(num_tests):
        ...
        file_ggd = os.path.join(test_dir, "twb_ggd.g2o")
...
if __name__ == "__main__":
    df_results = evaluate_batch_results(root_dir="test_results/tutorial_slam2d/exp1_test_results_1", save_path="test_results/tutorial_slam2d/exp1_test_results_1/ape_results.pkl", percentile = 0.7)
    print(df_results)
    ...
    mean_ape_ggd = np.mean(results_np[:, 1])

    print(f"Mean APE (Gaussian): {mean_ape_gaussian:.6f}")
    print(f"Mean APE (GGD):      {mean_ape_ggd:.6f}")

    max_ape_gaussian = np.max(results_np[:, 0])
    max_ape_ggd = np.max(results_np[:, 1])

    print(f"Max APE (Gaussian): {max_ape_gaussian:.6f}")
    print(f"Max APE (GGD):      {max_ape_ggd:.6f}")

    # 2. Compute std APEs (sample std; use ddof=0 for population)
    std_ape_gaussian = np.std(results_np[:, 0], ddof=1)
    std_ape_ggd      = np.std(results_np[:, 1], ddof=1)

    print(f"Std APE (Gaussian):  {std_ape_gaussian:.6f}")
    print(f"Std APE (GGD):       {std_ape_ggd:.6f}")

    # 2. Evaluate GGD improvement (per test, GGD must be strictly better)
    ggd_wins = np.sum(results_np[:, 1] < results_np[:, 0])
    total_tests = results_np.shape[0]
    improvement_ratio = ggd_wins / total_tests

    print(f"GGD wins in {ggd_wins} out of {total_tests} tests.")
    print(f"GGD Improvement Ratio: {improvement_ratio:.2%}")



    # Find indices where GGD is not better than Gaussian
    failures = df_results[df_results["APE_GGD"] >= df_results["APE_Gaussian"]]

    print(f"Number of failures: {len(failures)}")
    print(failures)
    t_stat, p_value = ttest_rel(df_results["APE_Gaussian"], df_results["APE_GGD"])
    print(f"Paired t-test: t={t_stat:.4f}, p={p_value:.4e}")
    w_stat, p_value = wilcoxon(df_results["APE_Gaussian"], df_results["APE_GGD"])
