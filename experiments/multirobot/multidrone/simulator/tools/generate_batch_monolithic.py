...
Each run n writes:
  ...
  - bot_ids.txt
  - msg_log_<id>.txt, gt_log_<id>.txt  (same outputs as tools/generate_single_run.py)
...
try:
    from simulators.multidrone.core.simulator import WorldSim
    from simulators.multidrone.core.trajectory_generator import trajectory_generation
except ModuleNotFoundError:
    # Allow direct execution: python .../tools/generate_batch_monolithic.py
    import sys

    sys.path.append(str(Path(__file__).resolve().parents[1]))
    from core.simulator import WorldSim
    from core.trajectory_generator import trajectory_generation

DEFAULT_CONFIG_PATH = "experiments/multirobot/multidrone/simulator/config/sim_config_lm_off.json"
DEFAULT_BATCH_ROOT = "test_data/multidrone/batch_unbounded_lm20_100"
...
def _run_single_simulation(
    *,
    ...
    """Mirror tools/generate_single_run.py: new trajectories + WorldSim + step loop."""
...
def generate_batch_multidrone_data(
    ...
    *,
    ...
    config_path
        Same sim_config.json used by tools/generate_single_run.py.
    batch_root
        Defaults to test_data/multidrone/batch (relative to process CWD).
    max_steps
        Upper bound on simulation steps per dataset (same as tools/generate_single_run.py loop).
    """
    if batch_count < 0:
