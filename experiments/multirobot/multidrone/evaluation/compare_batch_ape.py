...
GGD) plus a shared GT batch root.

Only the first N runs are compared, with N = min(# numeric run subdirs in batch root,
# numeric run subdirs in batch_noGgd root). Run ids are those in the intersection of both
...
try:
    from evaluators.multidrone.plot_trajectory_comparison import (
        compute_ape,
        read_gt_trajectory,
        read_tum_trajectory,
    )
except ModuleNotFoundError:
    # Allow direct execution: python .../evaluators/multidrone/compare_batch_ape.py
...
def _format_batch_roots_debug(
    ...
    s_ng = describe("batch_noGgd_root", no_ggd_root)
    ...
    if not s_b:
        lines.append(
            ...
            "path (e.g. typo batch_f2_nGgd vs batch_f2_noGgd)."
        )
    if not s_ng:
        lines.append(
            "  Hint: batch_noGgd_root has no numeric runs — path may be wrong or folder missing."
        )
    ...
    if s_gt and s_ng and not (s_gt & s_ng):
        lines.append(
            f"  gt ∩ batch_noGgd is empty. Only in gt: {sorted(s_gt - s_ng, key=int)[:15]}…"
        )
...
def main(
    ...
    batch_root: Path = Path("test_results/multidrone/repeats/batch_f2_bounded"),
    batch_no_ggd_root: Path = Path("test_results/multidrone/repeats/batch_f2_nGnd_bounded"),
    drone_ids: List[str] | None = None,
) -> None:
    ids_b = set(_numeric_subdirs(batch_root))
    ids_ng = set(_numeric_subdirs(batch_no_ggd_root))
    ...
    if not runs:
        raise RuntimeError(
            ...
            "gt / batch / batch_noGgd, after capping at N=min(#batch, #batch_noGgd).\n"
            + _format_batch_roots_debug(gt_batch_root, batch_root, batch_no_ggd_root)
        )
    print(
        f"Comparing first N={len(runs)} trajectories "
        f"(N_cap=min(batch={len(ids_b)}, batch_noGgd={len(ids_ng)})={n_cap}; "
        f"{len(common)} common with GT, numeric order {runs[0]}…{runs[-1]})."
    )

    batch_vals: List[float] = []
    no_ggd_vals: List[float] = []

    print("\nAPE mean (GT vs estimate), side-by-side")
    print("run | batch             | batch_noGgd")
    print("----+-------------------+-------------------")
    for r in runs:
        v_batch = _run_ape_mean(gt_batch_root, batch_root, r, drone_ids)
        v_noggd = _run_ape_mean(gt_batch_root, batch_no_ggd_root, r, drone_ids)
        batch_vals.append(v_batch)
        no_ggd_vals.append(v_noggd)
        b_str = f"{v_batch:.6f}" if np.isfinite(v_batch) else "nan"
        n_str = f"{v_noggd:.6f}" if np.isfinite(v_noggd) else "nan"
        print(f"{int(r):>3d} | {b_str:>17} | {n_str:>17}")

    batch_arr = np.asarray(batch_vals, dtype=float)
    no_ggd_arr = np.asarray(no_ggd_vals, dtype=float)
    print("\nOverall mean across runs:")
    print(f"  batch mean APE      = {np.nanmean(batch_arr):.3f} \pm {np.nanstd(batch_arr):.3f}")
    print(f"  batch_noGgd mean APE= {np.nanmean(no_ggd_arr):.3f} \pm {np.nanstd(no_ggd_arr):.3f}")

    valid_mask = np.isfinite(batch_arr) & np.isfinite(no_ggd_arr)
    valid_mask = valid_mask.astype(bool)
    paired_batch = batch_arr[valid_mask]
    paired_no_ggd = no_ggd_arr[valid_mask]
    
    print(f"\n GGD results are better in {np.sum(paired_no_ggd > paired_batch)} out of {paired_batch.size} tests.")

    print("\nPaired significance tests (batch vs batch_noGgd):")
    ...
    # H0: mean(batch - no_ggd) == 0
    # H1 (two-sided): mean(batch - no_ggd) != 0
    t_stat, t_p = scipy_stats.ttest_rel(paired_batch, paired_no_ggd, nan_policy="omit")
...
if __name__ == "__main__":
    # Note: on disk the no-GGD folder is typically `batch_f2_noGgd`, not `batch_f2_nGgd`.
    main(
        gt_batch_root=Path("test_data/multidrone2/RAL_mod/100_0lm_0midpoint_f2_test"),
        batch_root=Path("test_results/multidrone/RAL_mod/100_0lm_0midpoint_f2_test_gnd"),
        batch_no_ggd_root=Path("test_results/multidrone/RAL_mod/100_0lm_0midpoint_f2_test_ngnd"),
    )
