...
static int runOneBatchItem(
  ...
  const std::string view_config_path = "experiments/multirobot/multidrone/config/view_config.json";
  ...
  if (enable_visualization) {
    ...
    for (size_t i = 0; i < robotIds.size(); ++i) {
      ...
      std::string bot_view_config =
          "experiments/multirobot/multidrone/config/bot" + robotIds[i] + "_view.json";
      if (slamSystem) {
...
int main(int argc, char* argv[]) {
  ...
  try {
    const std::string batch_config_path =
        "experiments/multirobot/multidrone/config/batch_experiment_config.json";
    const std::string default_base_path =
        "experiments/multirobot/multidrone/config/experiment_base_config.json";
