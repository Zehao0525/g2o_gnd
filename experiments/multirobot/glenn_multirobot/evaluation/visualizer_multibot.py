...
if __name__ == "__main__":
    ...
    filename = "test_results/glenn_multirobot/multi_round/file_trajectory_gt_bot0.g2o"
    results_gt = read_se3_vertices_as_se2(filename)[:len_bot0+1]
    print("len(results_gt)", len(results_gt))

    filename = "test_results/glenn_multirobot/multi_round/file_trajectory_opt_bot0.g2o"
    results_ggd = read_se3_vertices_as_se2(filename)
    results_ggd_edge = results_ggd[:len_bot0+1]
    print("len(results_ggd)", len(results_ggd))


    filename = "test_results/glenn_multirobot/file_trajectory_opt_wognd_bot0.g2o"
    results_noggd = read_se3_vertices_as_se2(filename)
    results_noggd_edge = results_noggd[:len_bot0+1]
    print("len(results_ggd)", len(results_noggd_edge))

    filename = "test_results/glenn_multirobot/file_trajectory_pre_comm_bot0.g2o"
    result_before = read_se3_vertices_as_se2(filename)[:len_bot0+1]
    print("len(result_before)", len(result_before))

    filename = "test_results/glenn_multirobot/file_trajectory_pre_opt_bot0.g2o"
    result_before_opt = read_se3_vertices_as_se2(filename)[:len_bot0+1]
    print("len(result_before_opt)", len(result_before_opt))


    filename = "test_results/glenn_multirobot/bot0_observation_vtxs_refs.g2o"
    obs_vtxs_bot0 = read_se3_vertices_as_se2(filename)[:4]
    obs_vtxs_ids = np.array(obs_vtxs_bot0)[:,0]
    selected = [p for p in results_ggd if p[0] in obs_vtxs_ids]
    ...
    filename = "test_data/glenn_multirobot/test1_new_data/gt.tum"
    ...
    gauss_opt_pose_path = list_to_pose_path(results_noggd_edge)
    ggd_opt_pose_path = list_to_pose_path(results_ggd_edge)
    ...
    # ggd opt
    traj_ref, traj_est = sync.associate_trajectories(gt_pose_path, ggd_opt_pose_path)
    ape_metric = metrics.APE(metrics.PoseRelation.translation_part)
    ape_metric.process_data((traj_ref, traj_est))
    ...
    #plot_trajectory(result_before_opt, 'purple', False, 'ggd comms edge', alpha = alpha)
    plot_trajectory(results_noggd_edge, 'red', False, 'ggd comms edge', alpha = alpha)
    plot_trajectory(results_ggd_edge, 'blue', False, 'ggd comms edge', alpha = alpha)
