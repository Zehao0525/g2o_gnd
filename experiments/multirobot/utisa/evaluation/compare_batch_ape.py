...
Adapted from python/evaluators/multidrone/compare_batch_ape.py.

Parameters (globals):
  SIM_DURATION_SEC / DERIVE_SIM_DURATION / EXPERIMENT_JSON — same meaning as
  plot_trajectory_comparison.py (evaluation time window in simulation-relative seconds).
...
try:
    from evaluators.utisa.common import (
...
BATCH_ROOT_A = Path("test_results/utisa/mrclam7_batch")
BATCH_ROOT_B = Path("test_results/utisa/mrclam7_batch_alt")  # set to same as A if no B
...
EXPERIMENT_JSON: Path | None = Path(
    "test_results/utisa/mrclam7_batch/.batch_merged_configs/experiment_run_single.json"
)
# ----------------------------------------------------
