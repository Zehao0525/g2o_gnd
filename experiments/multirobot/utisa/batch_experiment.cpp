...
int main(int argc, char* argv[]) {
  ...
  try {
    const std::string batch_config_path = "experiments/multirobot/utisa/config/batch_experiment_config.json";
    const std::string default_base_path = "experiments/multirobot/utisa/config/experiment_base_config.json";
    ...
    const std::string view_config_path = batch_j.value(
        "view_config_path", std::string("experiments/multirobot/utisa/config/view_config.json"));
    const std::string view_config_dir = batch_j.value("view_config_dir", std::string("experiments/multirobot/utisa/config"));

    std::ifstream base_in(base_config_path);
    if (!base_in) {
      throw std::runtime_error(
          "cannot open experiment base config: " + base_config_path +
          " (run from g2o_example/ so experiments/... paths resolve)");
    }
    json base_j;
    base_in >> base_j;

    std::string slam_config_path = base_j.at("slam_config_path").get<std::string>();
    std::string topology_path = "experiments/multirobot/utisa/config/topology.json";
