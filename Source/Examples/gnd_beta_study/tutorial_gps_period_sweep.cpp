...
struct OptimizationConfig {
  ...
  int gndWarmupIterations = 10;
  int gndActiveIterations = 10;
  bool verbose = false;
};
...
struct GpsPeriodSweepConfig {
  ...
  std::string outputBaseDir = "test_results/gnd_beta_study/gps_period_sweep";
...
GpsKernelKind parseKernelKind(const std::string& type) {
  ...
  if (type == "gnd") return GpsKernelKind::GND;
...
GpsPeriodSweepConfig loadSweepConfig(const std::string& configPath) {
  ...
  if (j.contains("optimization")) {
    ...
    cfg.optimization.gndActiveIterations =
        o.value("gnd_active_iterations", cfg.optimization.gndActiveIterations);
  ...
  if (j.contains("kernels")) {
    ...
    cfg.kernels.gndPower = k.value("gnd_power", cfg.kernels.gndPower);
  ...
  if (cfg.kernels.robustDelta <= 0.0) {
    cfg.kernels.robustDelta = cfg.kernels.gndBound;
  }
...
RobustKernel* makeGpsKernel(const GpsKernelSpec& spec, const KernelParams& params,
  ...
  switch (spec.kind) {
    case GpsKernelKind::Gaussian:
      return nullptr;
    case GpsKernelKind::GND:
      return new ToggelableGNDKernel(params.gndBound, params.gndPower, params.gndLnc,
...
void assignGpsKernels(SparseOptimizer& optimizer, const GpsKernelSpec& spec,
                      const KernelParams& params, bool* gndActivePtr) {
  for (auto& edgePair : optimizer.edges()) {
    auto* gps = dynamic_cast<EdgePlatformLocPrior*>(edgePair);
    if (!gps) {
      continue;
    }
    gps->setRobustKernel(makeGpsKernel(spec, params, gndActivePtr));
  }
}
...
void runKernelOptimization(SparseOptimizer& optimizer, const GpsKernelSpec& spec,
                           const OptimizationConfig& optCfg, bool& gndActive) {
  if (spec.kind == GpsKernelKind::GND) {
    gndActive = false;
    optimizer.initializeOptimization();
    optimizer.optimize(optCfg.gndWarmupIterations);
    gndActive = true;
    optimizer.initializeOptimization();
    optimizer.optimize(optCfg.gndActiveIterations);
    return;
  }
...
int main(int argc, char* argv[]) {
  ...
  const std::string defaultConfigPath =
      "Source/Examples/gnd_beta_study/config/gps_period_sweep_config.json";
  ...
  for (int gpsPeriod : cfg.gpsPeriods) {
    ...
    for (const GpsKernelSpec& spec : cfg.kernelSpecs) {
      ...
      assignGpsKernels(optimizer, spec, kernelParams,
                       spec.kind == GpsKernelKind::GND ? &gndActive : nullptr);

      cerr << "Optimizing period " << gpsPeriod << " test " << testIdx << " kernel " << spec.name
           << endl;
      runKernelOptimization(optimizer, spec, optCfg, gndActive);
