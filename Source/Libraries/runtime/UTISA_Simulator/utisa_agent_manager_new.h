...
class UTISAAgentManagerNew {
 public:
  UTISAAgentManagerNew(const std::string& config_path,
                    const std::string& log_path,
                    const std::string& slam_config_path);
  ~UTISAAgentManagerNew();
  ...
  void saveLandmarks(const std::string& output_dir) const;
...
/// Perform topology-based communication between drones.
  ...
  UTISASlamSystemNew* getSlamSystem(size_t index) {
  ...
  UTISASimulator* getSimulation(size_t index) {
    if (index >= sims_.size()) {
      return nullptr;
    }
...
protected:
  ...
  std::vector<std::unique_ptr<UTISASimulator>> sims_;
  std::vector<std::unique_ptr<UTISASlamSystemNew>> slamSystems_;
  ...
  /// Forwarded into each `UTISASlamSystemNew` (per-observation sync entries in broadcast when true).
