...
UTISAAgentManagerNew::UTISAAgentManagerNew(const std::string& config_path,
                           ...
                           const std::string& slam_config_path) : verbose_(false)
{
  ...
  // 3. For each robot id, create a UTISASimulator and a UTISASlamSystemNew
  ...
  for (const auto& id : robotIds_) {
    ...
    slamSystems_.push_back(std::make_unique<UTISASlamSystemNew>(id, slam_config_path));
...
UTISAAgentManagerNew::~UTISAAgentManagerNew() = default;

void UTISAAgentManagerNew::configureNoInterRobotCommunication() {
...
void UTISAAgentManagerNew::platformEstimate(Eigen::Vector3d& x, Eigen::Matrix3d& P) {
 ...
 */
void UTISAAgentManagerNew::start() {
  {
    ...
    UTISASlamSystemNew::resetPreOptTrajectoryBatchCounter();
  }
 ...
 */
void UTISAAgentManagerNew::stop() {
  if (communicationEnabled_) {
...
void UTISAAgentManagerNew::dumpPreOptTrajectories() {
  if (!debugOutputs_) {
    return;
  }
  const int batch = UTISASlamSystemNew::takeNextPreOptTrajectoryBatchIndex();
 ...
 */
void UTISAAgentManagerNew::step() {
  step(dt_);
}

bool UTISAAgentManagerNew::keepRunning() const {
...
void UTISAAgentManagerNew::step(double dt) {
  // High-level intent:
  //  - advance each UTISASimulator by dt
  //  - collect generated events/messages
  //  - feed events into corresponding UTISASlamSystemNew
...
void UTISAAgentManagerNew::setTopology(const std::vector<std::pair<std::string, std::string>>& topology) {
  topology_.clear();
...
void UTISAAgentManagerNew::setTopologyJson(const std::string& topology_path) {
  std::ifstream in(topology_path);
  if (!in) {
    throw std::runtime_error("UTISAAgentManagerNew::setTopologyJson: cannot open topology file: " +
                             topology_path);
  }
...
void UTISAAgentManagerNew::getPoses(std::vector<std::vector<double>>& poses) {
...
void UTISAAgentManagerNew::saveTrajectories(const std::string& output_dir, const std::string& format) const {
...
void UTISAAgentManagerNew::saveLandmarks(const std::string& output_dir) const {
  std::filesystem::path dir_path(output_dir);
  std::error_code mkdir_ec;
  std::filesystem::create_directories(dir_path, mkdir_ec);
  if (mkdir_ec) {
...
void UTISAAgentManagerNew::optimiseSystem(int count) {
  for (auto& slam : slamSystems_) {
    if (!slam) continue;
...
void UTISAAgentManagerNew::performCommunication() {
