...
UTISASlamSystemNew::UTISASlamSystemNew(const std::string& id, const std::string& filename)
    : Base(id, filename) {
  std::ifstream f(filename);
  if (!f) {
    throw std::runtime_error("UTISASlamSystemNew: cannot open SLAM config: " + filename);
  }
  json j;
  f >> j;
  ...
  if (sensorOff.size() != 3) {
    throw std::runtime_error("UTISASlamSystemNew: sensor_offset must have exactly 3 values [x, y, theta]");
  }
  landmarkSensorOffset_ = SE2(sensorOff[0], sensorOff[1], sensorOff[2]);
}

UTISASlamSystemNew::~UTISASlamSystemNew() = default;

void UTISASlamSystemNew::start() {
  graphChanged_ = false;
  ...
  if (!optimizer_->addParameter(poseFrame)) {
    throw std::runtime_error("UTISASlamSystemNew: failed to register pose-frame ParameterSE2Offset");
  }
  ...
  if (!optimizer_->addParameter(lmSensor)) {
    throw std::runtime_error("UTISASlamSystemNew: failed to register landmark-sensor ParameterSE2Offset");
  }
}

void UTISASlamSystemNew::stop() {
  if (verbose_) {
...
void UTISASlamSystemNew::dumpPreOptTrajectory(const std::string& run_directory) {
...
void UTISASlamSystemNew::processEvent(Event& event) {
  graphChanged_ = true;
...
void UTISASlamSystemNew::handleInitializationEvent(UTISAInitEvent event) {
  if (verbose_) {
...
void UTISASlamSystemNew::handleOdometryEvent(UTISAOdomEvent event) {
  if (verbose_) {
...
void UTISASlamSystemNew::handleLMObservationEvent(UTISALmObsEvent event) {
  if (!initialized_) {
    if (verbose_) {
...
void UTISASlamSystemNew::handleObservationEvent(UTISAObsEvent event) {
  if (verbose_) {
...
UTSIAMessage UTISASlamSystemNew::broadcastSyncMessage() const {
...
UTSIAMessage UTISASlamSystemNew::handleObservationSyncRequest(UTSIAMessage& msg) {
  if (verbose_) {
...
void UTISASlamSystemNew::handleObservationSyncResponse(const UTSIAMessage& message) {
  if (verbose_) {
...
void UTISASlamSystemNew::platformEstimate(Eigen::Vector3d& x, Eigen::Matrix3d& P) {
  if (!currentPlatformVertex_) {
    x.setZero();
    P.setZero();
    return;
...
void UTISASlamSystemNew::platformEstimate(Eigen::Vector3d& pose) const {
  if (!currentPlatformVertex_) {
    pose.setZero();
    return;
  }
...
std::vector<std::pair<double, SE2>> UTISASlamSystemNew::getTrajectory() const {
...
void UTISASlamSystemNew::getRangeBearingObservationSegments(
...
void UTISASlamSystemNew::saveTrajectoryTUM(const std::string& filename) const {
...
void UTISASlamSystemNew::saveLandmarksXY(const std::string& filename) const {
  std::ofstream out(filename);
  if (!out) {
