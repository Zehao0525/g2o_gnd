...
/**
 * UTISA SLAM system rebased onto MultibotSlamSystem / SlamSystemBase (new).
 * Parallel to UTISASlamSystem; does not replace it.
 */
class G2O_TUTORIAL_SLAM2D_API UTISASlamSystemNew
 ...
 public:
  ...
  UTISASlamSystemNew(const std::string& id, const std::string& filename);
  ~UTISASlamSystemNew() override;
