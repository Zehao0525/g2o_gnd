...
/**
 * Glenn FileSlamSystem rebased onto MultibotSlamSystem / SlamSystemBase (new).
 * Parallel to FileSlamSystem; does not replace it.
 */
class G2O_TUTORIAL_SLAM2D_API FileSlamSystemNew
 ...
 public:
  ...
  FileSlamSystemNew(int id, const std::string& filename);
  ~FileSlamSystemNew() override;
