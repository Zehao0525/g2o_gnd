...
AgentManagerNew::AgentManagerNew(const std::string& config_path,
                           ...
                           const std::string& slam_config_path) : verbose_(false)
{
  // 1. Parse config JSON (for dt, possibly bots info)
  {
    ...
    // Forwarded into DSMessage.lm_query and consumed by MultiDroneSLAMSystemNew.
  ...
  // 3. For each robot id, create a DataBasedSimulation and a MultiDroneSLAMSystemNew
  ...
  for (const auto& id : robotIds_) {
    ...
    slamSystems_.push_back(std::make_unique<MultiDroneSLAMSystemNew>(id, slam_config_path));
...
AgentManagerNew::~AgentManagerNew() = default;

void AgentManagerNew::platformEstimate(Eigen::Isometry3d& x,
                                    Eigen::Matrix<double,6,6>& P)
{
  ...
  // If MultiDroneSLAMSystemNew has its own platformEstimate method, prefer that:
...
void AgentManagerNew::start() {
  {
    ...
    MultiDroneSLAMSystemNew::resetPreOptTrajectoryBatchCounter();
  }
...
void AgentManagerNew::dumpPreOptTrajectories() {
  if (!debugOutputs_) {
    return;
  }
  const int batch = MultiDroneSLAMSystemNew::takeNextPreOptTrajectoryBatchIndex();
 ...
 */
void AgentManagerNew::step() {
  step(dt_);
}

bool AgentManagerNew::keepRunning() const {
...
void AgentManagerNew::step(double dt) {
  // High-level intent:
  //  - advance each DataBasedSimulation by dt
  //  - collect generated events/messages
  //  - feed events into corresponding MultiDroneSLAMSystemNew
...
void AgentManagerNew::setTopology(const std::vector<std::pair<std::string, std::string>>& topology) {
  topology_.clear();
...
void AgentManagerNew::setTopologyJson(const std::string& topology_path) {
  std::ifstream in(topology_path);
  if (!in) {
    throw std::runtime_error("AgentManagerNew::setTopologyJson: cannot open topology file: " +
                             topology_path);
  }
...
void AgentManagerNew::getPoses(std::vector<std::vector<double>>& poses) {
...
void AgentManagerNew::saveTrajectories(const std::string& output_dir, const std::string& format) const {
...
void AgentManagerNew::optimiseSystem(int count) {
  for (auto& slam : slamSystems_) {
    if (!slam) continue;
...
void AgentManagerNew::performCommunication() {
